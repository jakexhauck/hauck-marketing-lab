...
import { NAV, deepLink, filterNav } from "@/lib/nav";
...
function Palette({ onClose }: { onClose: () => void }) {
  ...
  const { signOut, isOwner, can } = useAuth();
  ...
  const commands = useMemo<Cmd[]>(() => {
    ...
    for (const item of filterNav(NAV, { isOwner, can })) {
      list.push({
        id: `nav:${item.to}`,
        label: item.label,
        sub: item.hint,
  ...
  }, [query, leadsData, contactsData, theme, toggleTheme, signOut, isOwner, can]);
