...
// "overdue" when GHL reports a due-but-unpaid invoice past its due date.
function normalizeStatus(raw: string | undefined, dueDate?: string): string {
  ...
  if (dueDate) {
    ...
    if (!Number.isNaN(due) && due < Date.now() && s !== "paid") return "overdue";
  }
  return s ? "sent" : "draft";
}

function shapeInvoice(inv: GhlInvoice): ApiInvoice {
  return {
    id: inv._id ?? inv.id ?? "",
...
export const onRequestGet: PagesFunction<Env, string, ApiData> = async (ctx) => {
  ...
  let invoices = all.map(shapeInvoice).filter((i) => i.id);
