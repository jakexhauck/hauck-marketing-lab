...
- **One page shell (non-negotiable):** every client page uses the same structure — `<PageHeader>` (the "Paid Ads" header: Poppins 22px title + description + right-aligned actions) above a **full-width** content column with a 24px desktop gutter via `PAGE_CONTAINER` from `src/lib/layout.ts`. **No `max-w-…` cap or `mx-auto` on the page column** (that is the left/right "white space" we removed). `PageHeader` already renders the global controls (search, agency chat, notifications bell, account menu) top-right at `lg+`, so **never add a per-page top bar or your own search/bell/avatar**. Full reference + copy-paste skeletons: `command-center/app/docs/PAGE_LAYOUT.md`.
...
3. **Pages** (`src/routes/<section>/`): one file per tab. Wrap in `<Shell>`, put the content column in `PAGE_CONTAINER` (`src/lib/layout.ts`), open with `<PageHeader>` (title + description + right-aligned `actions`), then `components/ui` primitives (`Panel`, `PanelHeader`, `Badge`, `Button`, `EmptyState`, `Segmented`). Full-width, no `max-w` cap. Use semantic tokens: `text-text/muted/faint`, `border-border/divider`, `bg-surface/surface-2`, `text-brand-text`, `bg-brand-tint`, `bg-positive-tint`, `text-positive`, `shadow-brand`, and inline `style={{ backgroundImage: "var(--grad-brand)" }}` for gradient fills. See `docs/PAGE_LAYOUT.md`.
4. **Shared module** (`src/routes/<section>/shared.tsx`): glyphs/helpers, a `NotConnectedNotice` component, and `export const SECTION_CONTAINER = PAGE_CONTAINER;` (re-export the one shared container, do not hand-roll a `max-w` string).
