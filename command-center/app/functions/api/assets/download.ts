...
import { tenantIdForSlug, tenantCanAccess } from "../../lib/clientAssets";

// GET /api/assets/download?fileId=<id>
// Stream a shared file's bytes through the command center, so the browser never
// touches the agency Drive token. Native Google docs are exported to a portable
// format. Access is refused unless the file sits under a folder the agency has
// shared to THIS tenant (tenantCanAccess), mirroring the admin download but
// scoped to the client's own tenant.
export const onRequestGet: PagesFunction<Env, string, ApiData> = async (ctx) => {
  ...
  try {
    ...
    if (!(await tenantCanAccess(token, supabase, tenantId, fileId))) {
