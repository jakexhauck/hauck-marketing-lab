...
//
// ONE ROW PER DAY, opening to the ads that made it. meta_ad_days stores a row
// per ad per day, and drawing that raw made a three-ad day three rows: reading a
// day's spend meant adding them up by eye, and a day with nine ads buried the
// day itself. The per-ad detail is not gone, it is one click down and sorted by
// spend, so the ad taking the budget is the first thing read.
//
...
//
// Two things about the table's own shape:
//
// The sheet is INSET from the page gutter (sm:mx-4 lg:mx-10). PAGE_CONTAINER's
// px-5/px-6 is the app-wide gutter and is left alone; this is extra and only
// here, so the table reads as an object on the page rather than running edge to
// edge. It still spans the width, the columns are not packed to one side.
//
// shrink-0 on that wrapper is not optional. PAGE_CONTAINER is a flex column with
// flex-1, and a direct child carrying overflow-x-auto gets CLIPPED by
// flex-shrink rather than scrolling. That is what once rendered Paid Ads Results
// sliced in half.
    ...
    <Shell>
      <div className={PAID_ADS_CONTAINER}>
        ...
        {query.isError ? (
          ...
          <div className="shrink-0 overflow-x-auto rounded-lg border border-border sm:mx-4 lg:mx-10">
            <table className="w-full min-w-[880px] border-collapse text-[12.5px]">
                ...
                {days.map((day) => {
                  ...
                  return [
                    ...
                    >
                      ...
                      {/* Reach is unique people, so adding the ads together
                          counts anyone who saw two of them twice. Marked rather
                          than either hidden or printed as though it were exact. */}
                      <td
                    ...
                    ...(isOpen
                      ? day.ads.map((ad, i) => (
                          ...
                          >
                            {/* The ad set sits UNDER the ad name rather than in
                                its own cell. It had one, and when Day moved to
                                the far right every number on these rows slid a
                                column left of the day-row figure it belongs
                                beneath. One cell for the ad, and the rest line
                                up. */}
                            <td
              ...
              <tfoot>
                <tr className="border-t border-border text-[12px] font-semibold">
                  ...
                  {/* Never exact across more than one day, for the same reason
                      it is not across more than one ad. */}
