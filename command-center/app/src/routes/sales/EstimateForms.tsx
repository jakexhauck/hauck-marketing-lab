...
// estimate requests (Organic Pipeline, source = "Website Form"). Left rail =
// submissions, filterable by status; right = the selected lead with a top
// action bar (Call now / Schedule / Book visit / Other), SMS + Email threads,
// and a composer. Quotes are given on the phone, never over text, so the call
// flow lives front-and-centre in the action bar. Populated demo inbox in
// demo/preview; a real session shows the empty, not-connected state.
      ...
      <Shell>
        <div className="mx-auto flex min-h-0 w-full max-w-3xl flex-1 flex-col px-5 pb-12 pt-6 lg:px-8">
          <header className="mb-5">
            <h1 className="font-display text-[22px] leading-none text-text">Estimate Forms</h1>
            <p className="mt-1.5 text-sm text-muted">
              Estimate requests from your website, with auto email + SMS follow-up.
            </p>
          </header>
          <div className="mb-5">
            <NotConnectedNotice />
          ...
          <div className="flex flex-1 flex-col items-center justify-center rounded-[var(--radius-lg)] border border-dashed border-border bg-surface py-16 text-center">
            ...
            <p className="mt-3 font-display text-[15px] text-text">No submissions yet</p>
            <p className="mt-1 max-w-xs text-[13px] text-muted">
              When someone fills out an estimate form on your website, they will show up
              here with their reply, ready to call.
            </p>
          </div>
        </div>
      </Shell>
        ...
        <div className="grid min-h-0 flex-1 lg:grid-cols-[320px_1fr]">
          ...
          >
            <div className="px-5 pb-2 pt-5">
              <h1 className="font-display text-[17px] font-semibold text-text">Estimate Forms</h1>
              <p className="mt-0.5 text-[12px] text-muted">
                {leads.length} submissions · auto email + SMS follow-up
              </p>
            </div>
          ...
          >
            {selected ? (
              ...
              <div className="flex flex-1 items-center justify-center text-[13px] text-faint">
                Select a submission to see the conversation.
              </div>
            )}
          </section>
        </div>
