...
// --- what the wizard puts next to a city -------------------------------------
...
// "21 Aug". Short because it sits inside a chip next to the city name.
...
/**
 * The marker beside a city in the wizard, or null when there is nothing to say.
 *
 * A city we have never touched gets no marker at all: the list would otherwise
 * be a wall of "new" chips saying nothing. The two markers that exist are the
 * two that change the decision, both read against the trade currently selected:
 * already done for it, or worked for something else and still open for this one.
 */
export function cityNote(
  city: LeadCity | null,
  label: (id: string) => string,
): CityNote | null {
  if (!city) return null;
  const cover = cityCoverage(city);
  if (cover === "cold") return null;

  if (cover === "open") {
    return { tone: "open", text: city.niches.map(label).join(", ") };
  }

  const when = shortDate(city.lastRunAt);
  const what =
    cover === "leads"
      ? `${city.leads.toLocaleString()} lead${city.leads === 1 ? "" : "s"}`
      : "ran, nothing found";
  return { tone: cover, text: when ? `${what}, ${when}` : what };
}
